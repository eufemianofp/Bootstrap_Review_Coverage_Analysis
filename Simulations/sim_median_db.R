...
estimator_fn <- function(x, index, alpha){
  ...
  theta_hat <- median(x)  # estimator value
  x_star <- x[index]  # bootstrap resample
  
  boot_star2 <- boot(data = x_star, 
                     statistic = function(x_star, ind) { return(median(x_star[ind])) }, 
...
results_parallel <- foreach(i=1:R,
  ...
  for (j in ns) {
    ...
    boot_rep <- boot(data = x[1:j], function(x, ind) {return(median(x[ind]))}, R = B)
